...
void RenderContextEGL::CreatePbufferSurface()
{
...
void RenderContextEGL::InitializeEglContext()
{
    if (IsEglContextReady()) {
        return;
    }

    ROSEN_LOGD("RenderContextEGL Creating EGLContext!!!");
    eglDisplay_ = GetPlatformEglDisplay(0, EGL_DEFAULT_DISPLAY, NULL);
    if (eglDisplay_ == EGL_NO_DISPLAY) {
        ROSEN_LOGW("RenderContextEGL Failed to create EGLDisplay gl errno : %x", eglGetError());
        return;
    }

    EGLint major, minor;
    if (eglInitialize(eglDisplay_, &major, &minor) == EGL_FALSE) {
        ROSEN_LOGE("RenderContextEGL Failed to initialize EGLDisplay");
        return;
    }

    if (eglBindAPI(EGL_OPENGL_ES_API) == EGL_FALSE) {
        ROSEN_LOGE("RenderContextEGL Failed to bind OpenGL ES API");
        return;
    }
    ...
    if (!(ret && static_cast<unsigned int>(count) >= 1)) {
        ROSEN_LOGE("RenderContextEGL Failed to eglChooseConfig");
        return;
    }
    ...
    if (eglContext_ == EGL_NO_CONTEXT) {
        ROSEN_LOGE("RenderContextEGL Failed to create egl context %x", eglGetError());
        return;
    }
    ...
    ROSEN_LOGD("RenderContextEGL Create EGL context successfully, version %d.%d", major, minor);
}

void RenderContextEGL::MakeCurrent(EGLSurface surface, EGLContext context)
{
...
void RenderContextEGL::ShareMakeCurrent(EGLContext shareContext)
{
    eglMakeCurrent(eglDisplay_, eglSurface_, eglSurface_, shareContext);
}

void RenderContextEGL::ShareMakeCurrentNoSurface(EGLContext shareContext)
{
    eglMakeCurrent(eglDisplay_, EGL_NO_SURFACE, EGL_NO_SURFACE, shareContext);
}

void RenderContextEGL::MakeSelfCurrent()
{
    eglMakeCurrent(eglDisplay_, eglSurface_, eglSurface_, eglContext_);
}

EGLContext RenderContextEGL::CreateShareContext()
{
...
void RenderContextEGL::SwapBuffers(EGLSurface surface) const
{
    RS_TRACE_FUNC();
...
void RenderContextEGL::DestroyEGLSurface(EGLSurface surface)
{
...
EGLSurface RenderContextEGL::CreateEGLSurface(EGLNativeWindowType eglNativeWindow)
{
    if (!IsEglContextReady()) {
...
EGLint RenderContextEGL::QueryEglBufferAge()
{
...
void RenderContextEGL::DamageFrame(int32_t left, int32_t top, int32_t width, int32_t height)
{
}

void RenderContextEGL::DamageFrame(const std::vector<RectI> &rects)
{
}

void RenderContextEGL::ClearRedundantResources()
{
    RS_TRACE_FUNC();
    if (grContext_ != nullptr) {

...
int32_t AbilityDelegatorRegistry::GetSocketConfig()
{
    if (abilityDelegatorArgs_ != nullptr) {
        if (!abilityDelegatorArgs_->GetSocketConfig().empty()) {
            return std::stoi(abilityDelegatorArgs_->GetSocketConfig());
        }
    }
    return -1;
}

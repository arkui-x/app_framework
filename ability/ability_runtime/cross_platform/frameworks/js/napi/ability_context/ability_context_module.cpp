...
extern const char _binary_ability_context_js_start[];
extern const char _binary_ability_context_abc_start[];
#if !defined(IOS_PLATFORM)
extern const char _binary_ability_context_js_end[];
...
{
    if (buf != nullptr) {
        *buf = _binary_ability_context_js_start;
    }

    if (bufLen != nullptr) {
        *bufLen = _binary_ability_context_js_end - _binary_ability_context_js_start;
    }
}

// ability_context JS register
extern "C" __attribute__((visibility("default"))) void NAPI_application_AbilityContext_GetABCCode(
    const char** buf, int* buflen)
{
    if (buf != nullptr) {
        *buf = _binary_ability_context_abc_start;
    }
    if (buflen != nullptr) {
